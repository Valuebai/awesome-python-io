...
::===================修改下面的2个号码（UserX），如果测多人群聊，在后面添加,%User32%即可
rem 群聊
adb shell am start -a android.intent.action.SENDTO -d sms:"%User13%,%User32%" --es sms_body  "111"
ping -n 5 127.1 >nul

call Z982_message_content.bat

rem 群聊，建立新的群聊会话
adb shell am start -a android.intent.action.SENDTO -d sms:"%User32%,%User26%" --es sms_body  "111"
ping -n 5 127.1 >nul
